...
# freshy<-main[-samplex,]
# preprocessed_freshy<-predict(impute, freshy[,-1])
# pred_rpart<-data.frame(ground_truth=freshy$classe, prediction=predict(treebagFit_dotx,preprocessed_freshy))
# confusionMatrix(pred_rpart$prediction, pred_rpart$ground_truth)
#
# preprocessed_TESTING<-predict(impute, TESTING_net_new[,-31])
# pred_rpart<-data.frame(ground_truth=TESTING_net_new$classe, prediction=predict(treebagFit_dotx,preprocessed_TESTING))
# confusionMatrix(pred_rpart$prediction, pred_rpart$ground_truth)
...
plot(treebagFit_dotx, metric = "Accuracy", plotType = "level",
#plot(treebagFit_dotx, metric = "Kappa", plotType = "level",
     scales = list(x = list(rot = 90)), main="treebagFit_dotx")

(xx<-ggplot(treebagFit_dotx, main="treebagFit_dotx")+ggtitle("xx"))

best_within_1pct<-cbind(tolerance="within 1 pct", treebagFit_dotx$results[tolerance(treebagFit_dotx$results, metric = "Accuracy", tol = 1, maximize = TRUE),1:4])
best_within_2pct<-cbind(tolerance="within 2 pct", treebagFit_dotx$results[tolerance(treebagFit_dotx$results, metric = "Accuracy", tol = 2, maximize = TRUE),1:4])
best_within_3pct<-cbind(tolerance="within 3 pct", treebagFit_dotx$results[tolerance(treebagFit_dotx$results, metric = "Accuracy", tol = 3, maximize = TRUE),1:4])
best_within_4pct<-cbind(tolerance="within 4 pct", treebagFit_dotx$results[tolerance(treebagFit_dotx$results, metric = "Accuracy", tol = 4, maximize = TRUE),1:4])
best_within_5pct<-cbind(tolerance="within 5 pct", treebagFit_dotx$results[tolerance(treebagFit_dotx$results, metric = "Accuracy", tol = 5, maximize = TRUE),1:4])
